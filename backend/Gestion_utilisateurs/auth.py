...
def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    try:
        # On désactive la vérification de la signature au niveau du microservice 
        # car l'API Gateway (Keycloak) s'en est déjà chargé et a validé le token!
        payload = jwt.get_unverified_claims(token)
        
        # Le token Custom Python utilise "sub" pour l'email.
        # Le token Keycloak utilise "email" ou "preferred_username".
        email: str = payload.get("email") or payload.get("preferred_username") or payload.get("sub")
        
        if email is None:
            print("Email was None in payload")
