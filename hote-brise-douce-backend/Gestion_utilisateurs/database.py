...
if not DB_PASSWORD:
    print(" [WARNING] DB_PASSWORD is empty - attempting connection without password")
if not DB_HOST:
...
print(f" [INFO] Database Config: user={DB_USER}, host={DB_HOST}:{DB_PORT}, db={DB_NAME}")
print(f" [INFO] Connection URL: mysql+pymysql://{DB_USER}:{masked_pwd}@{DB_HOST}:{DB_PORT}/{DB_NAME}")
